---
name: agent-onboard
description: Bootstrap a coding agent on a new machine end-to-end: audit, install skills/CLIs, write instruction files, verify. Use when the user says init my agent, bootstrap, set up, configure my environment, what's missing, refresh my skills, or (中文) 开箱 / 初始化 / 配置 agent / 补全环境. Do NOT use for one missing piece (a CLI, a key, a single skill) — install it directly; for a read-only machine probe use env-detect.
license: Apache-2.0
---
