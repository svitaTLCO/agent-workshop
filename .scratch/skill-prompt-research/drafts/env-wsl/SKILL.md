---
name: env-wsl
description: Configure WSL2 as a complete home for coding agents. Use on WSL for paths, .exe bridging, Docker, VS Code remote, NVIDIA GPU passthrough (NVIDIA-only boundary), daemon persistence across VM restarts, networking modes (mirrored vs NAT, autoProxy leaks), USB devices via usbipd, or wslconfig tuning. On WSL-capable machines pairs with env-windows (profile windows-wsl) — native-Windows-only concerns live there.
license: Apache-2.0
---
...
- Standalone home with same-box parity with `env-windows`/`env-linux`/`env-macos`: every capability below has a native WSL path; routing decisions live at the user. Steer work to native Windows only for the sanctioned exits named under Workload ownership, always labeled with its costs.
