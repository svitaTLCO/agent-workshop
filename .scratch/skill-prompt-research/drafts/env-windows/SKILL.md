---
name: env-windows
description: Configure native Windows (pwsh, winget) as the complete home for coding agents: packages, profiles, toolchains, MSVC/vcvars native builds, daemons and local models (Ollama, scheduled tasks), Docker, encoding pitfalls, workload routing (Android/adb, embedded/IoT serial, USB devices). Use on Win32 or when the agent runs in PowerShell/cmd — inside WSL use env-wsl instead. On WSL-capable boxes the windows-wsl profile activates both skills. WSL is only ever a suggestion, never a requirement.
license: Apache-2.0
---
...
- Route by workload BEFORE starting a task — the environment follows the toolchain and hardware, there is no global default. The two-question rule (where is the toolchain? does the task touch physical/network equipment?), Android/embedded specifics, and crossing-the-boundary rules live in `references/workload-routing.md`; unclear → ask one question: where is the toolchain/device installed?
